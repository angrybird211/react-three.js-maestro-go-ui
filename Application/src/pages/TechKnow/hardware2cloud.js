...
const hardware_2_cloud = props => {
  ...
  useEffect(() => {
    ...
    axios
    .get("https://maestro2go.azurewebsites.net/api/hardware_2_cloud", {
    headers: { "Access-Control-Allow-Origin": "*", },
    })
    .then((res) => {
    setData2(res.data);
  ...
  const onNodeClick = function(node, event) {
    ...
    for (var entry in data[0]["nodes"]){
      if (data[0]["nodes"][entry].id == node.id){
        for (var entry2 in data2){
            console.log(entry2)
            console.log(data2[entry2])
            if (entry2 == data[0]["nodes"][entry].group){
                window.alert("For this cluster we recommend an azure VM instance " + data2[entry2].armskuName)
            }
        }
      }
    }
...
export default hardware_2_cloud;