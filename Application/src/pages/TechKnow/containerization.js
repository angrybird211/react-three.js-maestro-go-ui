...
const containerization_model = props => {
  ...
  useEffect(() => {
    ...
    axios
    .get("https://maestro2go.azurewebsites.net/api/get_containerization_model", {
    headers: { "Access-Control-Allow-Origin": "*", },
    })
    .then((res) => {
    setData2(res.data);
  ...
  const onNodeClick = function(node, event) {
    ...
    for (var entry in data2){
        for(var a in data2[entry]){

            var name = String(node.id).split(":")[0]
            if (String(data2[entry][a]).includes(name)){
                console.log(data2[entry][a])
                window.alert(data2[entry][a])
                return
            }
        }
...
export default containerization_model;